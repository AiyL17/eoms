...
@section('header-actions')
    <a href="{{ route('executive-orders.index') }}" id="tour-header-btn" class="btn-secondary btn-sm">
    ...
    <div class="block md:hidden divide-y divide-slate-100">
        ...
        <div class="px-4 py-4">
            <div class="flex items-start justify-between gap-3 mb-2">
                <span class="text-xs font-bold text-amber-700 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-lg font-mono">{{ $eo->eo_number }}</span>
                <span class="text-xs text-slate-400">{{ $eo->deleted_at->format('M d, Y') }}</span>
            </div>
            <p class="text-sm font-semibold text-slate-800 leading-snug mb-0.5">{{ Str::limit($eo->title, 70) }}</p>
            <p class="text-xs text-slate-400 mb-3 truncate">{{ Str::limit($eo->subject, 80) }}</p>
            <p class="text-xs text-slate-400 mb-3">Uploaded by: <span class="font-medium text-slate-600">{{ $eo->uploader->name ?? '—' }}</span></p>
        ...
        <div class="py-16 text-center px-4">
            ...
            <p class="text-sm text-slate-500 mb-5">{{ request()->anyFilled(['search']) ? 'Try a different search term.' : 'No archived executive orders found.' }}</p>
            @if(request()->anyFilled(['search']))<a href="{{ route('executive-orders.archive') }}" class="btn-secondary btn-sm">Clear Search</a>@endif
        </div>
        @endforelse
    </div>
    ...
    <div class="hidden md:block overflow-x-auto">
        <table class="w-full table-auto table-wide">
            <thead>
                ...
                @endphp
                <tr>
                    <th class="w-36"><a href="{{ $sortUrl('eo_number') }}" class="inline-flex items-center gap-1 group hover:text-violet-700 transition-colors">EO Number {!! $sortIcon('eo_number') !!}</a></th>
                    <th><a href="{{ $sortUrl('title') }}" class="inline-flex items-center gap-1 group hover:text-violet-700 transition-colors">Title & Subject {!! $sortIcon('title') !!}</a></th>
            ...
            <tbody>
                ...
                <tr class="group">
                    ...
                    <td>
                        <div class="text-[13px] font-semibold text-slate-800 mb-0.5 truncate max-w-xs" title="{{ $eo->title }}">{{ Str::limit($eo->title, 65) }}</div>
                    ...
                    <td class="text-right pr-5 whitespace-nowrap">
                        <div class="flex items-center justify-end gap-2">
                            <form action="{{ route('executive-orders.restore', $eo->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 px-3 py-1.5 rounded-lg transition-colors" title="Restore this EO">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" /></svg>
                                    Restore
                                </button>
                            </form>
                            <form action="{{ route('executive-orders.force-destroy', $eo->id) }}" method="POST">
                                ...
                                <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-semibold text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
                                        data-confirm="Permanently delete {{ $eo->eo_number }}? The PDF and all previous versions will also be removed. This cannot be undone." title="Permanently delete">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                                    Delete Forever
                                </button>
                            </form>
